...
export class DamageCalculator {
    constructor(attacker, defender, skill=null) {
        ...
        this.isSkill = false; // Using a skill or not
        this.skill = skill;
        this.damage = 0;
    ...
    getAttackType() {
        ...
        } else if (attacker instanceof Psykeeper) {
    ...
    applyDefense(attack) {
        ...
        switch (this.attackType) {
            case Moverutils.AttackType.MAGIC_SKILL:
                defense = this.applyMagicSkillDefense(attack);
                break;
            case Moverutils.AttackType.AUTO_ATTACK:
                defense = this.applyGenericDefense(attack);
                break;
            default:    // Melee skills
                defense = this.applyDefenseParryCritical(attack);
                if (damage > 0) {
    ...
    getDamageMultiplier() {
        ...
        if (this.isSkill) {
            ...
            if (this.skill.id == 5162 || this.skill.id == 8916) {
                if (this.attacker.hasBuff(7395)) // Dark Illusion
                factor *= 1.4;
            }
            ...
            switch (this.skill.id) {
                ...
                case 6206: // Spirit Bomb
                    factor *= Math.min(100.0 / 90.0, 1.0) * 2.0; // 100% mana
                    break;
            }
        ...
        if (delta > 0) {
            const reduceFactor = [1.0, 1.0, 0.98, 0.95, 0.91, 0.87, 0.81, 0.75, 0.67, 0.59, 0.51, 0.42, 0.32, 0.22, 0.12, 0.01]
     ...
     */
    afterDamage(damage) {
        if (this.isSkill && this.skill.id == 1947) { // Reflex hit
            const skillLevel = this.skill.levels.length;
    ...
    applyGenericDefense(attack) {
        ...
        if (damage > 0) {
            ...
            damage = Math.floor(damage * (1 - criticalChance / 100.0) + criticalDamage * (criticalChance / 100.0));
    ...
    applyMagicSkillDefense(attack) {
        ...
        // Apply the defense element
        const skillElement = Moverutils.Elements[this.skill.element];
        if (skillElement == Moverutils.Elements.none) {
            return damage;
        }

        damage = Math.floor(damage * (1.0 - this.getElementResist(skillElement)));

        // Get the attack weapon element
        let weaponElement = Moverutils.Elements.none;
        // TODO: Check weapon element upgrade here FIRST, then else:
        if (this.weapon.element) {
            weaponElement = Moverutils.Elements[this.weapon.element];
        }

        // 10% extra damage if the skill matches the weapon element
        if (skillElement == weaponElement) {
            return Math.floor(damage * 1.1);
        }

        // 10% less damage if the weapon element is weak compared to the skill element
        if ((weaponElement == Moverutils.Elements.water && skillElement == Moverutils.Elements.fire) ||
    ...
    getElementDamage() {
        ...
        if (false) {} // if element upgrade is not none
        else {
    ...
    applyAttackDefense(attack, defense) {
        ...
        const corr = Math.floor(Moverutils.lerp(defense, attack, value));
    ...
    computeDefense() {
        ...
        if (this.attackType == Moverutils.AttackType.MAGIC_SKILL) {
            if (this.attackContext == Moverutils.AttackContext.AC_PVE)
                ...
                defense = this.defender.getExtraParam("magicDefense");
    ...
    computeGenericDefense() {
        ...
        const equipmentDefense = this.defender.defense;

        const staFactor = 0.75;
        const levelScale = 2.0 / 2.8;
        const statScale = 0.5 / 2.8;

        let defense = Math.floor(level * levelScale + (this.defender.sta * statScale + (this.defender.sta - 14) * jobFactor) * staFactor - 4);
        equipmentDefense /= 4;
    ...
    getMeleeSkillPower() {
        ...
        const levelProp = skill.levels[skillLevel - 1];

        const weaponAttack = this.getWeaponAttackPower();
        
        const referStat = levelProp.scalingParameters.reduce((total, current) => total += this.attacker[current.stat] * current.scale, 0);

        let powerMin, powerMax;

        powerMin = ((weaponAttack[0] + (levelProp.minAttack + this.weapon.additionalSkillDamage) * 5 + referStat - 20) * (16 + skillLevel) / 13);
        powerMax = ((weaponAttack[1] + (levelProp.maxAttack + this.weapon.additionalSkillDamage) * 5 + referStat - 20) * (16 + skillLevel) / 13);
    ...
    getDamagePropertyFactor() {
        ...
        if (this.attackContext == Moverutils.AttackContext.AC_PVE) {
            ...
            defenseLevel = 0; // monster element attack, usually none
        }
        ...
        const Relation = {
            None,   // One has no element.
            Same,   // Same element.
            Weak,   // Attacker element is weak.
            Strong  // Attacker element is strong.
        };
        ...
        switch (result) {
            ...
            default:
                if (attackLevel > 0 && defenseLevel == 0) {
                    ...
                    factor += upgradeFactors.damage[Math.min(defenseLevel - 3, 10) - 1]; // Both damage and defense use the same table
                } else if (attackLevel > 0 && defenseLevel > 0) {
    ...
    computeAsalraalaikumDamage() {
        ...
        switch (skillLevel) {
			...
			case 7: add = 80; break;
			case 8: add = 90; break;
			case 9: add = 100; break;
			case 10: add = 150; break;
            default: return 0;
        }

        const mana = this.attacker.mp;

        return ((this.attacker.str / 10) * skillLevel) * (5 + mana / 10) + add;
    }
    ...
    getStatScale(levelProp, isPVP) {
        ...
        for (let scale of levelProp.scalingParameters) {
            if (scale.parameter == "attack" && scale[spellContext]) {
                ...
                switch(scale.stat) {
                    case "int":
                        statValue = attacker.int;
                        break;
                    case "sta":
                        statValue = attacker.sta;
                        break;
                    case "str":
                        statValue = attacker.str;
                        break;
                    case "dex":
                        statValue = attacker.dex;
                        break;
                }

                total += statValue * scale.scale;
                //total += (scale.scale / 10.0 * statValue) + (skillLevel * statValue / 50.0);
            }
        }
    ...
    getDamagePerSecond() {
        let dps = 0;
        let hitsPerSecond = 0;
        const damage = this.damage > -1 ? this.damage : this.computeDamage();

        if (this.attackType == Moverutils.AttackType.AUTO_ATTACK) {
            const hitRate = this.getHitRate();
            const attackSpeed = this.attacker.aspd;
            hitsPerSecond = (this.attacker.constants.hps * attackSpeed) * (hitRate / 100.0);
            ...
            hitsPerSecond = (30.0 / frames) * (this.attacker.DCT) / (cooldown + 1);
     ...
     */
    getTimeToKill() {
        return this.defender.hp / this.getDamagePerSecond();
    }
